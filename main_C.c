...
int main(void)
{
  ...
  void read_file_and_send_uart(const char *filename) {
      ...
      FATFS fs;         // Estructura para el sistema de fileNames
...
transmit_uart("File opened for reading.\n");
      ...
      do {
          fres = f_read(&fil, dataBuf, sizeof(dataBuf) - 1, &bytesReceived);
          switch(fres == FR_OK) {
 case 1:
...
transmit_uart("Root directory opened.\n");
      ...
      while (1) {
          fres = f_readdir(&dir, &fno);  // Leer el siguiente fileName
          switch(fres != FR_OK || fno.fname[0] == 0) {
 case 1:
...
static void MX_SPI1_Init(void)
{
  ...
  switch(HAL_SPI_Init(&hspi1) != HAL_OK) {
 case 1:
